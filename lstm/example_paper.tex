...
Submission to SysML 2019 will be entirely electronic, via a web site
(not email). Information about the submission process and \LaTeX\ templates
are available on the conference web site at:
\begin{center}
\textbf{\texttt{https://www.sysml.cc/}}
\end{center}
...
\textbf{Anonymous Submission:} SysML uses double-blind review: no identifying
author information may appear on the title page or in the paper
itself. Section~\ref{author info} gives further details.

\textbf{Simultaneous Submission:} SysML will not accept any paper which,
at the time of submission, is under review for another conference or
has already been published. This policy also applies to papers that
overlap substantially in technical content with conference papers
under review or previously published. SysML submissions must not be
submitted to other conferences during SysML's review period. Authors
may submit to SysML substantially different versions of journal papers
...
\texttt{nohyperref} as one of the options to the \texttt{sysml2019}
...
The footnote, ``Preliminary work. Under review by the SysML. Do not distribute.'' must be
modified to ``\textit{Proceedings of the
$\mathit{2}^{nd}$ SysML Conference},
Palo Alto, CA, USA, 2019.
Copyright 2019 by the author(s).''

For those using the \textbf{\LaTeX} style file, this change (and others) is
handled automatically by simply changing
$\mathtt{\backslash usepackage\{sysml2019\}}$ to
$$\mathtt{\backslash usepackage[accepted]\{sysml2019\}}$$
...
head using the \texttt{fancyhdr} package which is included in the SysML
2019 style file package. In case that the original title exceeds the
size restrictions, a shorter form can be supplied by using

\verb|\sysmltitlerunning{...}|
...
SysML uses double-blind review, so author information must not appear. If
you are using \LaTeX\/ and the \texttt{sysml2019.sty} file, use
\verb+\sysmlauthor{...}+ to specify authors and \verb+\sysmlaffiliation{...}+ to specify affiliations. (Read the TeX code used to produce this document for an example usage.) The author information
...
to be submitted
as Supplementary Material via CMT\@. However, keep in mind that an SysML
...
A sample file with author names is included in the SysML2019 style file
...
facility, use \texttt{natbib.sty} and \texttt{sysml2019.bst}
