...
%\subsection{Overview}
Figure~\ref{fig:flow} shows the overall compilation and mapping process. 
...
To convert from the hierarchical model to a streaming dataflow graph, we duplicate and distribute nested loop controllers for each VB, as shown in Figure~\ref{fig:controller}(b). 
...
We use a bipartite graph between VBs and PBs to indicate valid matches between the two, where an edge between a VB and PB suggests a valid mapping. 
...
%Such cost metrics can be either quantitative (such as number of fan-in and fan-out for control, scalar, and vector variables, memories with different types (FIFO, scratchpad, etc.), pipeline stages and registers) or qualitative (e.g. whether PB is connected to DRAM controller, with/out support of reduction and floating-point operations). 
%These cost metrics include:
%\begin{itemize}
    %\item Fan-in and -out counts
    %\item Types of memories
    %\item Number of pipeline stages and registers
    %\item Reduction support
...
%We use a community detection algorithm with random walk from the \emph{igraph} library \cite{igraph} to partition the data flow graph within each VB. 
%The resulting partitions must form a directed acyclic graph---if they depend cyclically on each other, then they will deadlock.

%We can further prune the partial bipartite graph from resource pruning by finding VB sub bipartite graphs (VB-SBG). 
%%We define SCBG graph within BG where no VB in the SCBG connect to any other PB outside the SCBG a maximal complete bipartite graph for VB (VB-maximal).
%The VB-SBG is a subgraph that that contains one or more VBs, and all PBs they connect to.
%As shown in Figure \ref{fig:prune}, iff we can find a VB-SBG whose number of VBs is less than the number of PBs, then we can terminate the matching process early, or continue partitioning VBs from VB-SBG, because there cannot be a match that satisfies every VB in the SCBG. 
%If we find a VB-SBG whose number of VBs equals to number of PBs, we can exclude these PBs from matching other VBs, as doing so will incur non-maximum matching among VBs within the VB-SBG. 
%This is less extensive checking compared to the Arc Consistency Algorithm \#3 (AC-3) \cite{mackworth1981consistency} but does not require examining pair-wise constraints for each combination. 
%With the observation that most VBs will map to the same set of PBs, we can find VB-maximal SCBGs quickly by going through the sets of PBs each VB is mapped to, and thus complete the pruning in $O(|VB|+|PB|)$ time.

%\begin{figure}
  %\centering
  %\includegraphics[width=0.85\columnwidth]{figs/prune.pdf}
  %\footnotesize
  %\begin{algorithmic} 
    %\algnotext{EndFor}
    %\algnotext{EndIf}
    %\If{ $\exists bg \subset BG | \Call{isVBMaximal}{bg} \And |\Call{VB}{bg}| > |\Call{PB}{bg}|$ }
      %\State { Terminate matching or continue partitioning }
    %\Else 
      %\For { $ bg \subset BG | \Call{isVBMaximal}{bg} \And |\Call{VB}{bg}| = |\Call{PB}{bg}|$ }
        %\For { $v \in {BG \setminus bg} $ }
          %\For { $p \in {BG \setminus bg}$ }
          %$BG -= Edge[v,p]$
          %\EndFor
        %\EndFor
      %\EndFor
    %\EndIf
  %\end{algorithmic}
  %\caption{Maximal pruning}\label{fig:prune}
%\end{figure}
...
%For example, for register \emph{N} in \emph{VB-D}, it is enqueued every cycle when producer \emph{C0-1} is enabled and dequeued every time \emph{C1.done} is high. 
...
\if 0
\subsubsection{Back-tracking + A* search Placement and Routing}
\begin{figure}[!ht]
  \small
  \begin{description}
...
%\subsubsection{Iterative Placement}
