...
Finally, \Cref{sec:net_dse} perform a detailed evaluation across the identified design space for a variety of benchmarks.

Because CGRAs encompass a broad range of architectures, we narrow our study on tiled-based CGRAs with
...
\Cref{sec:heuristic}.
%This knowledge allows the compiler to accurately determine which network flows in the transformed 
%design correspond to throughput-critical inner-loop traffic and which correspond to low-bandwidth outer-loop traffic.
%}
...
in choosing a CGRA network, using benchmarks showin in \Cref{tab:benchmark}.
...
%\begin{figure}[ht]
%\centering
%\includegraphics[width=\textwidth]{figs/area.pdf}
