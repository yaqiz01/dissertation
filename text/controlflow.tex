...
hardware. 
\Cref{fig:contextalloc} shows an example of the context allocation. For simplicity, we use the same
example as \Cref{fig:spatialegpar} without outer loop parallelization on loop \texttt{A}.
...
\subsection{Synchronization Allocation} 
\label{sec:sync}
...
\end{table*}
\paragraph{Where.}
\name only allocate resource to synchronize contexts if they contain declared accesses on the same
memory that can potentially interfere.
...
multiple concurrent access streams through multiple DAGs. From the programmer perspective, users do not need
to serialize the two contexts reading the same DRAM address. SRAMs, on the other hand, has a single read 
and write port. Programmers must guarantee that a PMU receives request from a single context
at any point for correctness.
