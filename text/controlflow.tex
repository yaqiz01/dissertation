...
To eliminate the round-trip latency, \name further divides each memory access in the input IR into 
a requesting and a receiving context, as shown in \Cref{fig:contextalloc} (c). 
For a read access, \name maps the address generation in a separate requesting context, streaming addresses and
data between the requesting context, the memory, and the receiving context.
...
For any two accesses enclosed by a loop, there could be a loop-carried dependency (LCD) from
...
access contexts to ensure the access order is consistent with the program.
...
For queues, the token is generated and consumed every cycle when the producer and receiver contexts are active.
For scalar variable and arrays, the program order expects that the producer/consumer
updates/inspects the memory for every iteration of their LCA controller.
...
\paragraph{Where.}
\name can serialize every accesses pair of a memory to enforce program order. However, that
would requires $N^2$ tokens for a memory with $N$ accesses ($2N^2$ with LDCs), which can be
unnecessarily expensive.
The last question is how can we minimize number of inserted token while ensuring a correct execution.
...
\Cref{fig:depeg} shows examples of dependency graphs for different control hierarchy and memory
types.
...
graph, enforcing the same ordering as the original graph.

Next, \name checks if each backward edge can be removed without breaking the execution order of
accesses. 
Each backward edge represents a loop-carried dependency (LCD) and is tagged with the associated loop.
An backward edge can be removed, if there is a path from the source to the destination
of the edge, after the edge is removed, 
with any forward edges remained from the previous pass plus \emph{a single} backward edge 
associated with the same loop.
\Cref{fig:graphred} shows examples of the dependency graph reduction.
...
credits in the destination buffers.
