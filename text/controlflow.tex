\section{Imperative to Dataflow Transformation}
\label{sec:control}
...
\caption[Access dependency graph]{
  ...
  \texttt{R0} and \texttt{R1} do not depend on each other because they are both DRAM read accesses as explained in \Cref{tab:interferetab}.
}
\label{fig:depeg}
\end{figure*}
...
graph, enforcing the same access ordering as the original dependency graph.
...
This way, the controllers under a branch does not need to execute if the outer branch clause
is disabled.
...
\texttt{A} before
waiting for the \emph{else} contexts. For the second iteration of loop \texttt{A} ($A_1$), the \emph{if} contexts
...
enabled. More interestingly, \Cref{fig:branch} (d) shows an overlapping execution of the
\emph{if} and \emph{else} clauses across iterations of A. 
Both \emph{if} and \emph{else} contexts are active almost all the time if loop \texttt{D}
and \texttt{F} runs for a large number of iterations.
This branch has a 50\% split between the \emph{if} and \emph{else} clauses. As a result, the
\emph{if} contexts only executes even iterations of \emph{A} and the \emph{else} contexts only
executes odd iterations of \emph{A}, each corresponding to half of \emph{A}'s iterations.
If the latency of loop \texttt{D} and \texttt{F} are both $L$,
the total runtime for $N$ iterations of loop \texttt{A} would be on the order of $\frac{N}{2}L$ due
to overlapping between the even and odd iterations between the \emph{if} and \emph{else} contexts, 
assuming L and N are large.
...
Instead of inlining the function call, there is a single copy of the hardware allocated for each
declared function, reused across multiple function calls.
The function hardware can be viewed as a shared resource like memories; 
contexts containing callers of the function must pass tokens around to serialize their accesses to the shared hardware.
We need a similar dependency graph described in~\Cref{sec:controlalloc} to ensure the calling order
among different callers is program-consistent.
The order matters because the function call might be stateful and has side effects.
