...
SRAMs, on the other hand, have a single read 
and write port. Programmers must guarantee that a PMU receives read requests from a single context
...
\subsection{Data-Dependent Control Flow (WIP)}
...
\Cref{fig:dynrange} shows an example of data-dependent loop ranges. \name uses a context to compute
...
writer contexts immediately sends out the token to the receiver
...
After all contexts and shared resources are allocated and synchronized, we simply put each context and shared resource into their own VBs.
The contexts with single-cycle special case (\Cref{sec:sync}) must be put in the same VB as the shared memory.

\subparagraph{Specialization for dense on-chip array}
