...
\Cref{tab:partprob}.
The partitioner ``fixes'' the VU based on a single PU specification, albeit there are many potential PUs 
...
To pipeline at full-throughput, \name needs to insert retiming buffers along imbalanced data path across
partitions.
Retiming introduces new VUs in addition to partitioned VUs, which attributes to cost in
