...
\begin{figure}
  ...
  \caption[Example PCU configuration]{
    ...
    In (b), we move the definitions of streams outside of the context, so they can be written by
    other contexts. However, these streams represent the input buffers of the context, where each
    stream can have exactly one writer and one reader.
  }
  \label{fig:context}
\end{figure}
...
\emph{This feature enable Plasticine to support complex control hierarchy, such as  non-perfectly nested loops and
...
consistent to the program order from a high-level imperative programming language.
...
synchronizations across these streams to preserve memory ordering expected by the program.
...
mapped onto a physical unit (PU), such as a PCU and PMU.
Each VU can contain multiple contexts. However, a VU can be mapped to a PU only if the aggregated
resource usage of all contexts in the VU can fit in the PU. The hardware can also limits the maximum 
number of contexts a PU can support, and has resources that cannot be split across contexts.
Additionally, messages across VUs are mapped across the global network, and must tolerate arbitrary 
amount of network latencies; messages within a single VU across contexts takes only a single cycle.
The transformation phase generates a virtual unit data-flow graph (VUDFG) with appropriate
...
The output of the resource allocation phase is a VUDFG with tagged PU type for each VU.
It is up to the
