...
\caption{
  ...
  \emph{B} that ocurr earlier in the program order due to network latency between \emph{B} and
  \emph{mem}.
...
To address this challenge, we want to eliminate the centralized outer controllers.
...
port. 
\name generates control tokens among the distributed writers to time-share the write port and 
enforce the ordering between writers.
