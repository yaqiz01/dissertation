...
development cycle, which hinder their accessibility to the general software community.
...
parallel workers or cache misses that generate messages for coherence protocol; these traffic are bursty and
...
overall performance is limited by the pipeline stage with lowest throughput.
...
This means as we going to larger chip size, the network bandwidth must grow super linearly 
to achieve a perfect performance scaling, unless exploring pipeline parallelism.
...
requiring expertise knowledge in hardware architecture to efficiently target the accelerator.
On the other side,
...
with a end-to-end integration with these frameworks.
...
These frameworks treats operations in the ML dataflow graph as blackbox kernels and implement
...
In the blackbox approach, each fused operation and data format corresponds to a new blackbox
kernel.
As a result, the combination of required templates easily becomes untractable.

The alternative whitebox approach showin in~\Cref{fig:bigpic} (b) uses a succinct front-end
...
constraints.
The framework then uses a series of transformations and optimizations to reformulate the kernel 
to an imperative implementation that can be supported by the accelerator. 
...
Next, the compiler can easily translates the imperative IR to the imperative front-ends of different
accelerators.

To enable an easy integration of the whitebox approach, we introduce a compiler--\name--that raise
...
Plasticine is a purely dataflow accelerator without a centralized scheduler that
...
the programmability of Plasticine, but also enables cross kernel optimizations on Plasticine.
