...
parallelized loops, \name maps the memory to the input buffer of an context, and setup datapath to
...
operations.
Fully parallelized loops is a common outcome of an automatic design space exploration on the parallelization factors.
...
(M2), \name{} eliminates the intermediate access and feed the output of M1's writer (W1) directly to M2's
reader (R2) if it can proof the propagation is safe.
...
Eliminating route-through
pattern can simplify control hierarchy and reduce number of basic blocks, which eliminates contexts.
...
\Cref{fig:mlp} shows an interesting case study on a MLP presenting performance scaling and resource
increase while scaling up parallelism.
\Cref{fig:mlp} (b) demonstrates that Plasticine can have resource
non-linearly increasing with parallelization factors, while having perfect performance 
scaling, due to compiler optimizations.

\subparagraph{Read address duplication (dupra).} 
%The long traveling path for BI from the address context to the receiver context showing in memory partitioning
%\Cref{sec:memsplit} \Cref{fig:memsplit} can cause pipeline stalls due to the imbalance datapath between
%the BI forwarding path and the memory data path.
...
within the receiver context, which could use less resources than retiming.
...
quantity limit.
%The traversal-based algorithm requires a reference cost from PU to check if the merged VU still satisfy the hardware constraint.
...
A common loop optimizations on traditional CPU is to move loop-invariant expressions 
...
Therefore, the reversed loop invariant hoisting, which moves loop-invariant expressions into the 
loop body, can sometimes saving resources by simplifying the control structure, as shown in 
...
\Cref{fig:heat} presents the evaluation on performance improvement and resource reduction from
